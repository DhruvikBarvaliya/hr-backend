 ...
 * - Configurable weekend days via env
 *    WEEKEND_DAYS (comma-separated list of numbers 0..6, where 0=Sunday).
 *
 * Exports:
...
async function isHolidayDate(input) {
  ...
  const found = await Holiday.findOne({
...
async function countBusinessDays(startInput, endInput) {
  ...
  // Build date strings list
  const days = [];
  for (let d = new Date(start); d <= end; d.setUTCDate(d.getUTCDate() + 1)) {
    days.push(dateToYMD(d));
  }

  // Query holidays that fall in this range (UTC bounds)
  ...
  // Count business days (no `continue`)
  let count = 0;
  for (let i = 0; i < days.length; i += 1) {
    ...
    const isHoliday = holidaySet.has(dayYmd);
...
async function listBusinessDays(startInput, endInput) {
  ...
  for (let i = 0; i < days.length; i += 1) {
    ...
    const isWeekend = WEEKEND_DAYS.includes(dObj.getUTCDay());
